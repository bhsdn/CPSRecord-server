...
export class TextCommandsService {
  ...
  async findAll(query: QueryTextCommandDto) {
    ...
    })).map((command: any) => this.mapCommand(command));
  }
  ...
  async create(dto: CreateTextCommandDto) {
    ...
    return this.prisma.textCommand
      ...
      .then((command: any) => this.mapCommand(command));
  }

  async bulkCreate(dto: BulkCreateTextCommandDto) {
    const uniqueSubProjectIds = Array.from(
      new Set(dto.commands.map((item) => item.subProjectId)),
    );

    await Promise.all(uniqueSubProjectIds.map((id) => this.ensureSubProjectExists(id)));

    const payload = await BatchOperations.batchCreateTextCommands(
      this.prisma,
      dto.commands,
    );
  ...
  async update(id: number, dto: UpdateTextCommandDto) {
    ...
    return this.prisma.textCommand
      ...
      .then((updated: any) => this.mapCommand(updated));
  }
